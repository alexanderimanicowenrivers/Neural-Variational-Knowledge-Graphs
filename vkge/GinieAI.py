...
class GinieAI:
    ...
    def TFKMeansCluster(self,vectors, noofclusters):
        ...
        with graph.as_default():
            ...
            ##values
            centroid_value = tf.placeholder([dim],dtype=tf.float32)
            cent_assigns = []
            ...
            mean_input = tf.placeholder([None, dim],dtype=tf.float32)
            ...
            v1 = tf.placeholder([dim],dtype=tf.float32)
            v2 = tf.placeholder([dim],dtype=tf.float32)
            ...
            centroid_distances = tf.placeholder([noofclusters],dtype=tf.float32)
